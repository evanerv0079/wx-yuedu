...
<script>
	...
	export default {
		...
		onLoad:function(e){
			_self = this;
			signModel.sign(this.appServer);
			loginRes = this.checkLogin('../write/write', '2');
			// 加载文章分类
			uni.request({
				url: _self.appServer+'category/getcategories',
				method: 'POST',
				success: res => {
					console.log(res)
						...
						uni.request({
							...
							success: res => {
								...
								if(res.data.status == 'ok'){
									...
									_self.artList = [];
									// 清空数据
									signModel.sign(_self.appServer);
									// 防止数据缓存
